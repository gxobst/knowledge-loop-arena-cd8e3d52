...
export async function callAIGateway(prompt: string, isJSONMode = false): Promise<string> {
  ...
  const hasLocalSettingsKey = isConfigured(s);
  const hasLocalKey = !!mistralLocalKey || hasLocalSettingsKey;

  if (hasLocalKey) {
    // PATH 1 (Local Override)
    if (mistralLocalKey) {
      try {
        ...
        console.warn("Direct Mistral client fetch failed:", e);
        throw e;
      }
    } else {
      // User-configured settings key
      try {
        ...
        if (s.provider === "anthropic") {
          ...
          // generic openai-like custom provider
        ...
        console.warn("Custom key API connection failed:", e);
        throw e;
      }
    }
  } else {
    // PATH 2 (Secure Default Cloud Proxy)
    try {
      ...
      console.warn("Secure backend proxy fetch failed:", e);
    }

    // PATH 3 (Frictionless Fallback)
